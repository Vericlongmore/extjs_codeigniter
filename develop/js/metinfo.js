...
function adminp(l, t) {
	if (langar[l] == langar[t]) {
	} else {
		window.location.href = 'index.php?lang=' + t;
	}
}

/*function commercial_license() {
	$.ajax({
		url : 'system/authcode.php?lang=' + lang
				+ '&autcod=1&cs=1&action_ajax=1',
		type : "GET",
		success : function(data) {
			if (data == 0) {
				var txt = '未' + '购' + '买' + '商' + '业' + '授' + '权';
				if (!window.atop) {
					data = txt;
				} else {
					data = atop == '' ? txt : atop;
			...
			$(".top-" + "r-t").append(
					'<div id="met' + 'info' + '_lice' + 'nse"></div>');
			var met = $("#metinfo_license");
			met.html('<a href="javascript:;" title="' + data
					+ '" id="license_isok">' + data + '</a>');
			met.find('a').css({
				'color' : '#f2fb02'
			});
			$('#license_isok').live('click', function() {
				$('#nav_1').click();
				$('#nav_1_15').click();
			});
		}
...
lg.click(function() {
	lt.empty().append(
			'<div style="text-align:center; color:#666;">' + user_msg['jsx1']
					+ '</div>');
	if (lt.is(':hidden')) {
		$.ajax({
			type : "POST",
	...
	lt.is(':hidden') ? $("#langcig").addClass('nowt') : $("#langcig")
			.removeClass('nowt');
